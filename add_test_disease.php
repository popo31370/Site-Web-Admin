...
<div class="container mt-5">
        ...
        <h2 class="mb-4">Ajouter un test et procédure à la maladie ...</h2>

        <form action="add_item_disease_post.php" method="post">
            <input id="id" name="id" type="hidden" value="<?= $_GET['id']; ?>">
            <div class="row">
                <div class="col-md-6">
                    <select id="id_proc" name="id_proc" class="form-control mb-2">
                        <!-- Remplir la combo box avec les données de l'API -->
                        <?php
                        if (isset($response) && is_array($response)) {
                            foreach ($response as $test) {
                                $nom = htmlspecialchars($test['nomFr'], ENT_QUOTES, 'UTF-8');
                                echo "<option value='{$test['id']}'>{$nom}</option>";
                            }
                        }
                        ?>
                    </select>
