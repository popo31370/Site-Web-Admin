...
<div class="container mt-5">
        ...
        <h2 class="mb-4">Ajouter un symptôme à la maladie ...</h2>

        <form action="add_item_disease_post.php" method="post">
            <input id="id" name="id" type="hidden" value="<?= $_GET['id']; ?>">
            <div class="row">
                <div class="col-md-6">
                    <select id="id_symp" name="id_symp" class="form-control mb-2">
                        <!-- Remplir la combo box avec les données de l'API -->
                        <?php
                        if (isset($response) && is_array($response)) {
                            foreach ($response as $symptome) {
                                $nom = htmlspecialchars($symptome['nomFr'], ENT_QUOTES, 'UTF-8');
                                echo "<option value='{$symptome['id']}'>{$nom}</option>";
                            }
                        }
                        ?>
                    </select>
                ...
                <div class="col-md-6">
                    <!-- Pain index input -->
                    <input type="number" id="pain_index" name="pain_index" class="form-control mb-2" placeholder="Pain index" required>
                </div>
                <div class="col-md-1">
